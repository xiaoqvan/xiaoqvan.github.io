...
这里是 xiaoqvan 的个人主页，使用 vue3 + vite + TS 构建
...
在 `src/config.ts` 中的 `socialLinks` 可以自定义社交链接。
更换 `href` 为自己的链接

```ts
  socialLinks: [
      { href: "#", title: "添加QQ", icon: "qq" },
      { href: "#", title: "bilibili", icon: "bilibili" },
      { href: "#", title: "前往GitHub", icon: "github" },
      { href: "#", title: "前往x-twitter", icon: "x-twitter" },
      { href: "#", title: "前往Telegram", icon: "telegram" },
    ],
```
...
> 请在 `src/config.ts` 中的 `playerId` 中填写你自己的明月浩空播放器 id
...
使用自定义外部链接的可以在 `src/config.ts` 中的 `backgroundImg` 中自定义.
